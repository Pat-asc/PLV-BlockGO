...
const VisibilityIcon = ({ visible }) => visible ? (
  <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" className="h-4 w-4" aria-hidden="true">
    <path d="M3 3l18 18M10.6 10.7a2 2 0 0 0 2.7 2.7M9.9 4.2A10.8 10.8 0 0 1 12 4c5.5 0 9 6 9 6a17.8 17.8 0 0 1-2.1 2.8M6.6 6.6C4.3 8.1 3 10 3 10s3.5 6 9 6c1 0 2-.2 2.8-.5" />
  </svg>
) : (
  <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" className="h-4 w-4" aria-hidden="true">
...
const PasswordField = ({ label, value, onChange, visible, onToggle, ariaLabel, required = false }) => (
  <label className="text-xs font-semibold text-slate-600">
    ...
    <span className="relative mt-1 block">
      <input
        ...
        value={value}
        onChange={onChange}
        className="w-full rounded-lg border border-slate-300 px-3 py-2 pr-11 text-sm font-normal"
      />
      <button
        type="button"
        aria-label={`${visible ? 'Hide' : 'Show'} ${ariaLabel}`}
        onClick={onToggle}
        className="absolute inset-y-0 right-0 inline-flex min-h-10 min-w-10 items-center justify-center rounded-r-lg text-slate-500 hover:bg-slate-100 hover:text-slate-800 focus:outline-none focus:ring-2 focus:ring-inset focus:ring-blue-600"
      >
...
const RegistrarAccountManagement = () => {
  ...
  const resetPassword = async (account) => {
    ...
    try {
      ...
      setPasswordVisibility((current) => ({ ...current, [account.id]: { password: false, confirmPassword: false } }));
    } catch (error) { setNotice({ type: 'error', message: error.message }); }
    finally { setSaving(false); }
  };
  ...
  return <div className="space-y-6">
    ...
    <section className="rounded-xl border border-slate-200 bg-white p-5 shadow-sm">
      <div className="flex flex-wrap items-start justify-between gap-3"><div><h2 className="text-xl font-bold text-[#003366]">Create Registrar Account</h2><p className="mt-1 text-sm text-slate-500">At most five current, non-deleted Registrar accounts are allowed.</p></div><span className={`rounded-full px-3 py-1 text-sm font-bold ${canCreate ? 'bg-emerald-100 text-emerald-800' : 'bg-amber-100 text-amber-800'}`}>{accounts.length} / {MAXIMUM_REGISTRAR_ACCOUNTS} Registrar accounts</span></div>
    ...
    <section className="rounded-xl border border-slate-200 bg-white p-5 shadow-sm">
      ...
      {loading ? <p className="py-8 text-center text-slate-500">Loading accounts…</p> : <div className="space-y-4">{accounts.map((account) => {
        ...
        const visibility = passwordVisibility[account.id] || {};
        const toggleVisibility = (field) => setPasswordVisibility((current) => ({ ...current, [account.id]: { ...visibility, [field]: !visibility[field] } }));
